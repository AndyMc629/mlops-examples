...
with DAG(
    ...
    # Fetching variables
    output_data_path = Variable.get("output_data_path", default_var="s3://mlops-pipeline-example/inference/predictions.csv")
    ground_truth_data_path = Variable.get("ground_truth_data_path", default_var="s3://mlops-pipeline-example/ground-truth/bank.csv")
    ...
    run_trigger_alerts = BashOperator(
        task_id='run_trigger_alerts',
        bash_command='echo triggering alerts ...'
    )
    
    log_monitor_data = BashOperator(
        task_id='log_monitor_data',
        bash_command='echo logging monitor data ...'
    )
