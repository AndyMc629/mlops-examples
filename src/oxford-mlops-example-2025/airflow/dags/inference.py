...
with DAG(
    ...
    # Fetching variables
    landing_data_path = Variable.get(
        "landing_data_path", default_var="s3://mlops-pipeline-example/landing/bank.csv"
    )
    ...
    load_models = BashOperator(
        task_id="load_models", bash_command='echo "Loading models..."'
    )
    ...
    log_inference_data = BashOperator(
        task_id="log_inference_data", bash_command='echo "Logging inference data..."'
    )
